      ...
      <section className='border-b border-cream-200 bg-cream-50 py-16 sm:py-20'>
        <Container>
          <SectionHeading
            ...
            eyebrow='Directory'
            title='Find a Lawyer'
            description={`Search and filter ${lawyers.length} lawyer${lawyers.length >= 1 ? 's' : ''} profiles across ${practiceAreas.length} practice${practiceAreas.length >= 1 ? 's' : ''} areas by name, location, practice area, and rating.`}
          />
        </Container>
      </section>
