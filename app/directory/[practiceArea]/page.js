...
export default async function PracticeAreaPage({ params }) {
  ...
  const matchingLawyers = lawyers.filter((lawyer) => lawyer.practiceAreas.includes(slug));
  const relatedAreas = practiceAreas.filter((item) => item.slug !== slug).slice(0, 3);
          ...
          <Container>
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
              {relatedAreas.map((related) => (
                <PracticeAreaCard key={related.id} area={related} />
              ))}
            </div>
          </Container>
        </section>
      ...
      <section className="bg-navy-900 py-16 text-cream-50">
        <Container className="flex flex-col items-start gap-4">
          ...
          <p className="max-w-xl text-cream-100/80">
            Browse the full directory to search across every practice area, location, and
            rating.
          </p>
          <Button href="/directory" variant="light">
