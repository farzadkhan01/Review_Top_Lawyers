...
export default function ImageTextSection({
  ...
  return (
    <div className={cn("grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-16", className)}>
      <div className={reverse ? "lg:order-2" : "lg:order-1"}>{children}</div>
      <div className={reverse ? "lg:order-1" : "lg:order-2"}>
        <Reveal delay={0.05}>
          <PlaceholderImage src={image} alt={imageAlt} sizes="(min-width: 1024px) 50vw, 100vw" />
        </Reveal>
      </div>
    </div>
  );
