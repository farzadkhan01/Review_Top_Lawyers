...
export default function LawyerFilters({
  ...
  const [practiceArea, setPracticeArea] = useState(lockedPracticeArea ?? initialPracticeArea);
  const [location, setLocation] = useState("");
  const [minRating, setMinRating] = useState("");
  const [sort, setSort] = useState(SORT_VALUES.includes(initialSort) ? initialSort : "rating");
  ...
  const results = useMemo(() => {
    ...
    if (practiceArea) {
      list = list.filter((lawyer) => lawyer.practiceAreas.includes(practiceArea));
    }
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-end">
          ...
          {!lockedPracticeArea && (
            <div className="lg:w-48">
              <FieldLabel htmlFor="directory-practice-area">Practice Area</FieldLabel>
              <select
